...
static void print_perms(const struct stat *sb){
  char read[10], write[10], execute[10]; 
  printf("Mode:                     %lo (octal)\n", (unsigned long) sb->st_mode & (S_IRWXU | S_IRWXG | S_IRWXO));
  if (sb->st_mode & S_IRUSR) strcpy(read, "yes"); 
  else strcpy(read, "no");
  if (sb->st_mode & S_IWUSR) strcpy(write, "yes"); 
  else strcpy(write, "no");
  if (sb->st_mode & S_IXUSR) strcpy(execute, "yes"); 
  else strcpy(execute, "no");
...
static void print_owner(const struct stat *sb){
  printf("Ownership:                UID=%s   GID=%s\n", getpwuid((long) sb->st_uid)->pw_name, getgrgid((long) sb->st_gid)->gr_name);
}
/* -------------------------------------------------------------------------------- */
